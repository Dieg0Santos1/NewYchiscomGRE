...
describe('GRE Traslado routes', () => {
  it('genera preview T002 con motivo 13 sin llamar declaracion', async () => {
    ...
    await request(app)
      ...
      .expect((response) => {
        ...
        expect(response.body.payload.motivoTraslado).toBe('13');
  ...
  it('rechaza el motivo 02 compra retirado del formulario', async () => {
    const app = createApp({
      config: { ...testConfig, dryRun: true, directDbInsertEnabled: false },
      greTrasladoService: {
        getNextSerie: vi.fn(),
    ...
    await request(app)
      ...
      .expect((response) => {
        expect(response.body.error).toBe('VALIDATION_ERROR');
        expect(response.body.issues[0].path).toBe('traslado.motivoTraslado');
      });
  });

  it('permite motivo 17 traslado de bienes para transformacion', async () => {
    const app = createApp({
      config: { ...testConfig, dryRun: true, directDbInsertEnabled: false },
      greTrasladoService: {
        getNextSerie: vi.fn(),
    ...
    await request(app)
      ...
      .send({
        ...
        traslado: {
          ...validTrasladoInput.traslado,
          motivoTraslado: '17',
          descripcionMotivoTraslado: 'TRASLADO DE BIENES PARA TRANSFORMACION'
        }
      })
      .expect(200)
      .expect((response) => {
        expect(response.body.payload.motivoTraslado).toBe('17');
      });
  });
