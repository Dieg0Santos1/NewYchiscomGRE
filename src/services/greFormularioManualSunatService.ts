...
const REQUIRED_RESPONSE_STATE = {
  blEstadoRegistro: 'L',
  processState: '_2_CONSULT'
} as const;
const ALLOWED_RESPONSE_PROCESS_STATES = ['SIGNED/ED_06', 'SIGNED/PE_02'] as const;
...
export type ManualSunatAcceptanceResult = {
  operationId: string;
  serieNumeroGuia: string;
  reused: boolean;
  updated: boolean;
...
export interface GreFormularioManualSunatService {
  setAcceptedMessage(serieNumeroGuia: string, options: { user?: string }): Promise<ManualSunatAcceptanceResult>;
}

export function buildManualAcceptedSunatMessage(serieNumeroGuia: string) {
  return JSON.stringify({
    codigo: '0',
    mensaje: `El Comprobante numero ${serieNumeroGuia}, ha sido aceptado`
  });
}

export function isEligibleForManualSunatMessage(row: Pick<ResponseRow, 'bl_estadoRegistro' | 'bl_estadoProceso' | 'process_state' | 'bl_mensajeSunat'>) {
  return row.bl_estadoRegistro === REQUIRED_RESPONSE_STATE.blEstadoRegistro
    && ALLOWED_RESPONSE_PROCESS_STATES.includes(row.bl_estadoProceso as typeof ALLOWED_RESPONSE_PROCESS_STATES[number])
    && row.process_state === REQUIRED_RESPONSE_STATE.processState
    && !row.bl_mensajeSunat?.trim();
}

export class DirectDbGreFormularioManualSunatService implements GreFormularioManualSunatService {
  constructor(private readonly config: AppConfig) {}

  async setAcceptedMessage(serieNumeroGuia: string, options: { user?: string }): Promise<ManualSunatAcceptanceResult> {
    ...
    try {
      ...
      await acquireAppLock(greFcTransaction, `GRE_FC_MANUAL_SUNAT_TRACE:${serieNumeroGuia}`);
      ...
      await acquireAppLock(bizlinksTransaction, `GRE_FC_MANUAL_SUNAT_RESPONSE:${serieNumeroGuia}`);
      ...
      if (before.bl_mensajeSunat?.trim() === acceptedMessage) {
        await insertEvent(greFcTransaction, traced, 'SUNAT_MENSAJE_MANUAL_REUTILIZADO', 'La guia ya tenia el mensaje manual de aceptacion SUNAT', {
          serieNumeroGuia,
      ...
      await insertEvent(greFcTransaction, traced, 'SUNAT_MENSAJE_MANUAL_INICIADO', 'Actualizacion manual controlada de bl_mensajeSunat iniciada', {
        serieNumeroGuia,
        user: options.user ?? null,
        before
      });
      ...
      await insertEvent(greFcTransaction, traced, 'SUNAT_MENSAJE_MANUAL_APLICADO', 'bl_mensajeSunat actualizado de forma controlada', {
        serieNumeroGuia,
        user: options.user ?? null,
        before,
        after
