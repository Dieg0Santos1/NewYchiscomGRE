...
/*
  Reproceso de GRE T002 publicas rechazadas por:
  - 7786: Para Transporte Publico solo debe enviar Fecha de entrega de bienes al transportista.
  - 7757: Codigo de establecimiento anexo del punto de partida no declarado.

  Accion:
  - Respaldar cabecera y respuestas actuales.
  - Para modalidad 01, dejar solo fechaEntregaBienes dentro de los campos especiales de traslado publico.
  - Limpiar fechaInicioTraslado, codigoPtoPartida y codigoPtollegada.
...
IF OBJECT_ID(N'dbo.ZZ_GRE_T002_REPROCESO_7786_20260911_DESPATCH', N'U') IS NULL
BEGIN
  SELECT TOP (0)
    ...
    d.*
  INTO dbo.ZZ_GRE_T002_REPROCESO_7786_20260911_DESPATCH
  FROM dbo.SPE_DESPATCH d;
END;

IF OBJECT_ID(N'dbo.ZZ_GRE_T002_REPROCESO_7786_20260911_RESPONSE', N'U') IS NULL
BEGIN
  SELECT TOP (0)
    CAST(NULL AS datetime2(0)) AS respaldadoEn,
    CAST(NULL AS sysname) AS respaldadoPor,
    CAST(NULL AS varchar(40)) AS lote,
    r.*
  INTO dbo.ZZ_GRE_T002_REPROCESO_7786_20260911_RESPONSE
  FROM dbo.SPE_DESPATCH_RESPONSE r;
END;

INSERT INTO dbo.ZZ_GRE_T002_REPROCESO_7786_20260911_DESPATCH
SELECT @Ahora, SUSER_SNAME(), 'GRE_T002_7786_20260911', d.*
FROM dbo.SPE_DESPATCH d
INNER JOIN @Series s ON s.serieNumeroGuia = d.serieNumeroGuia
WHERE d.tipoDocumentoGuia = '09';

INSERT INTO dbo.ZZ_GRE_T002_REPROCESO_7786_20260911_RESPONSE
SELECT @Ahora, SUSER_SNAME(), 'GRE_T002_7786_20260911', r.*
FROM dbo.SPE_DESPATCH_RESPONSE r
...
BEGIN
  THROW 51200, 'Hay series sin cabecera SPE_DESPATCH. No se reprocesa.', 1;
END;
...
BEGIN
  THROW 51201, 'Alguna serie parece tener aceptacion/ticket/CDR. No se reprocesa.', 1;
END;
...
SELECT
  ...
  d.codigoPtoPartida,
  d.codigoPtollegada,
  d.numeroDocumentoPtoPartida,
  d.numeroDocumentoPtoLlegada,
...
BEGIN
  COMMIT TRANSACTION;
  SELECT 'COMMIT_APLICADO' AS resultado, 'Guias reencoladas como Transporte Publico solo con fechaEntregaBienes.' AS detalle;
END
ELSE
BEGIN
